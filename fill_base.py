        ...
        for i in res['tracks']:
            for j in i['artists']:
                ...
                try:
                    ...
                    connection.execute(f'''INSERT INTO artists_by_genres(artist_id, genre_id) VALUES('{artist_id}', '{genre_id}')''')
                except sqlalchemy.exc.IntegrityError:
...
def fill_albums(connection, sp):
    ...
    for artist in artists_list:
        ...
        for album in albums['items']:
            ...
            try:
                connection.execute(f'''INSERT INTO albums(album_name, album_year, sp_id) VALUES('{al_name}', {al_year}, '{sp_id}')''')
...
def fill_album_artists(connection, sp):
    ...
    for album in album_list:
        ...
        for al_artist in album_artists:
            ...
            artist_data = connection.execute("SELECT id FROM artists WHERE sp_id = '" + al_artist['id'] +"'").fetchone()
            if artist_data == None:
               ...
               existed_genre = connection.execute(f"SELECT genre_id FROM genres WHERE name = '{genre_name}'").fetchone()

               if existed_genre == None:
                   connection.execute(f'''INSERT INTO genres(name) VALUES('{genre_name}')''')
                   artist_info['genre_id'] = connection.execute("SELECT genre_id FROM genres WHERE name = '" + genre_name +"'").fetchone()
               ...
               try:
                ...
                try:
                    connection.execute(f'''INSERT INTO albums_artists(album_id, artist_id) VALUES('{album[0]}', '{artist_id}') ''')
                except sqlalchemy.exc.IntegrityError:
                    print('такая запись уже имеется в таблице')
...
def fill_tracks(connection, sp):
    ...
    for album in album_list:
        ...
        for track in track_list:
            ...
            album_id = album[0]
            try:
                connection.execute(f'''INSERT INTO tracks(sp_id, track_name, track_duration, album_id) VALUES('{sp_id}', '{track_name}', {track_duration}, {album_id})''')
            except sqlalchemy.exc.IntegrityError:
                   print('такая запись уже имеется в таблице')
...
def create_my_compilation(connection, sp):
    ...
    for i in compilation_list:
        ...
        connection.execute(f'''INSERT INTO Compilation_albums(ca_name, ca_year) VALUES('My compilation №{last_ca_id}',{random.randint(2010, 2022)})''')
